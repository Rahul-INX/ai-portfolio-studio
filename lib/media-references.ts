...
export async function mediaReferences(url: string) {
  ...
  const [project, caseStudy, experiment, blog, dashboard, achievement, document, profile, markdown] = await Promise.all([
  ...
  return { total: project + caseStudy + experiment + blog + dashboard + achievement + document + profile + markdown, public: publicProject + publicCaseStudy + publicExperiment + publicBlog + publicDashboard + publicAchievement + publicDocument + profile + markdown };
}
